...
主界面顶部显示PLC连接、PLC当前工位、当前产品和待检测数量。左侧“数据查询”支持按日期、工位、产品和条码筛选历史检测记录；页面内的“导出检测记录”可单独选择日期、产品和工位后导出完整结果。

程序支持 ATEQ 侧漏仪 RS232 打印机模式结果架构，可从 PLC 读取的气密原始字符串中解析程序号、测试压力、泄漏值、单位及仪器状态，并与原始字符串一起入库。如果结果为 `AL` 报警，仪器未输出有效泄漏值时，界面显示 `--` 并显示对应报警原因。
...
- 历史记录保存在 `Data\mydb.db`，程序不会自动删除主数据库中的检测记录。
- 默认每 24 小时在 `Data\Backups` 生成一份 SQLite 在线备份，保留最近 7 天，可在 `Config.ini` 的 `[Database]` 中调整。
- 默认每 24 小时执行一次完整性检查；数据库所在磁盘低于 10240 MB 时弹出告警。
- “数据查询”页面内的“导出检测记录”采用逐行流式导出，不受界面最多显示 5000 条限制，也不会将全部历史记录一次性载入内存。
...
- 软件首次连接 PLC 时会将工位 1、2、3 的条码绑定状态点位 D3000、D3002、D3004 清零。扫码绑定成功后对应点位写 1，记录成功入库后写回 0；写入 1 或回读验证失败时取消本次绑定并报警。
