...
namespace AirtightInspection.Forms
{
    public sealed class ManualViewerForm : UIForm
    {
        ...
        private readonly Button _previous, _next, _zoomIn, _zoomOut;
        ...
        public ManualViewerForm(string folder, string product)
        {
            ...
            _zoomOut = UiFactory.Button("缩小", (_, __) => { _zoom -= 10; ApplyZoom(); });
            _zoomIn = UiFactory.Button("放大", (_, __) => { _zoom += 10; ApplyZoom(); });
        ...
        {
            if (_current == null) return; _zoom = Math.Max(50, Math.Min(300, _zoom));
            _picture.Size = new Size(_current.Width * _zoom / 100, _current.Height * _zoom / 100);
        ...
        private void UpdateButtons()
        {
            ...
            _zoomOut.Enabled = _current != null && _zoom > 50; _zoomIn.Enabled = _current != null && _zoom < 300;
